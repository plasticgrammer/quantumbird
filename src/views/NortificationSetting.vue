<template>
  <v-container class="notification-settings-container">
    ...
    <v-card class="notification-settings-card" outlined>
      <v-form
        ref="form"
        v-model="isFormValid"
      ...
      >
        <v-row class="mt-2">
          <v-col cols="12">
            <v-text-field
              v-model="notificationSettings.sender"
              label="送信元メールアドレス"
              outlined
              dense
              :rules="[
        ...
        <v-row>
          <v-col cols="12">
            <v-card outlined>
              <v-card-title>通知を行う曜日</v-card-title>
              <v-card-subtitle>指定曜日に週次報告依頼メールを送信します。</v-card-subtitle>
              <v-card-text>
                <v-row no-gutters>
                  <v-col
                  ...
                  >
                    <v-radio
                      v-model="notificationSettings.dayOfWeek"
                      :label="day.text"
                      :value="day.value"
        ...
        <v-row class="mt-4">
          <v-col cols="12" sm="6">
            <v-select
              v-model="notificationSettings.hour"
              :items="hours"
              item-text="text"
              item-value="value"
              label="通知時間（時）"
...
const { notificationSettings, loading, isFormValid, isFormChanged } = toRefs(state)
...
const validateForm = async () => {
  ...
  isFormValid.value = validation.valid && !!notificationSettings.value.dayOfWeek
}
...
const handleSubmit = async () => {
  ...
  try {
    ...
    showNotification('通知設定を更新しました')
    console.log('Notification settings updated:', notificationSettings.value)
    state.originalSettings = JSON.parse(JSON.stringify(notificationSettings.value))
    isFormChanged.value = false
  } catch (error) {
    showNotification('通知設定の保存に失敗しました', error)
  } finally {
    loading.value = false
  }
}
...
onMounted(async () => {
  ...
  try {
    ...
    showNotification('通知設定の取得に失敗しました', error)
  } finally {
    loading.value = false
  }
})
